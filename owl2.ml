...
let is_concept_name_class_option c =
  match c with
    Class.Constructor.IRI i -> Some (IRI.str_of i)
  | Class.Constructor.Thing -> None
  | _ -> failwith "Constructor not implemented."
...
let is_concept_name_class_expression_option ce =
  match ce.data with
    ClassExpression.Constructor.Class c -> is_concept_name_class_option c
  | ClassExpression.Constructor.ObjectIntersectionOf (_, _) -> None
  | ClassExpression.Constructor.ObjectSomeValuesFrom (_, _) -> None
  | _ -> failwith "Constructor not implemented."
(*
...
let is_domain_exists_concept_name_expression exp =
  match exp.data with
    ClassExpression.Constructor.ObjectSomeValuesFrom (rexpf, cexp)-> 
        (match rexpf.data with
          ObjectPropertyExpression.Constructor.ObjectProperty rexp ->
            (match rexp.data with
               ObjectProperty.Constructor.IRI i ->
               (match i.data with IRI.Constructor.IRI r ->
                 (match is_concept_name_class_expression_option cexp with
                    Some cname -> Some (r, cname)
                  | None -> None))
             | _ -> None)
         | _ -> None)
  | _ -> None
...
let rec is_conjunction_of_concept_names_expression ce =
  try
    match ce.data with
      ClassExpression.Constructor.Class c -> None
    | ClassExpression.Constructor.ObjectSomeValuesFrom (_, _) -> None
    | ClassExpression.Constructor.ObjectIntersectionOf (conj1, conj2) ->
      let l = ref [] in
      let _ = match is_concept_name_class_expression_option conj1 with
                Some cname ->  l := cname::!l;
              | None -> match is_conjunction_of_concept_names_expression conj1 with
                          Some l2 -> l := List.rev_append !l l2
                        | None -> raise Not_found;
      in
      let _ =  match is_concept_name_class_expression_option conj2 with
                Some cname -> l := cname::!l;
               | None -> match is_conjunction_of_concept_names_expression conj2 with
                          Some l2 -> l := List.rev_append !l l2
                        | None -> raise Not_found;
      in
      Some !l
