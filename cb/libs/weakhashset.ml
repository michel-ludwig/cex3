...
(*|        end;                                        *)
(*|  in                                                *)
(*|  h.index.(h.rover) <- shrink h.index.(h.rover);    *)
...
let shrink_bucket h =
  ...
  let rec shrink = function
    ...
    | n :: rest ->
        let idx = n mod dtsize in
        if Weak.check h.data idx then
          match shrink rest with
          | Some result -> Some (n :: result)
          | result -> result
        else begin
(*|          Printf.fprintf stderr "lost %n\n" idx;*)
          h.recycle <- idx :: h.recycle;
          h.size <- pred h.size;
          match shrink rest with
          | None -> Some rest
  ...
  in
  begin match shrink h.index.(h.rover) with
    | Some result ->
        h.index.(h.rover) <- result;    
    | _ -> ()
  end;
  h.rover <- succ h.rover mod (Array.length h.index)
    ...
    let rec find = function
      ...
      | n :: next ->
          if n / dtsize = k then
            ...
            | Some elt_tst as elt_o when H.equal elt elt_tst -> elt_o
            | _ -> find next
          else find next
    in
